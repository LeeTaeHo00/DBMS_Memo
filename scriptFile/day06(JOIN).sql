...
-- 등가 조인 : 두 개 이상의 테이블을 조인할 때 특정 컬럼들의 값이 같은 행만 결과로 반환
...
-- 3) 두개의 테이블에서 일치하느 컬럼을 확인 후 ON절ㅇ 추가
ON TB.BOOK_ID  = TR.BOOK_ID;
-- 선행테이블의 행이 적을 수록 효율적이다.

-- 회원의 이름과 대여한ㄷ 책의 제목을 가져오는 등가조인
-- 선행테이블과 후행테이블을 확인
 ...
 SELECT MEM_NAME, BOOK_TITLE, TR.REN_RENTALDATE, TR.REN_RETURNDATE
 FROM TBL_RENTAL TR
 JOIN TBL_BOOKS TB ON TR.BOOK_ID = TB.BOOK_ID
 ...
 -- 비등가 조인 : 두 테이블간의 조건이 같음이 아닌 범위 조건등을 사용하는 방식
 -- 책이름과 빌려간 날짜가 2023-12-20 일 부터 2023-12-23 사이에 있는지 조회
 
 SELECT BOOK_TITLE, TR.REN_RENTALDATE, TR.REN_RETURNDATE
 FROM TBL_RENTAL TR
 JOIN TBL_BOOKS TB ON TR.REN_RENTALDATE 
 BETWEEN to_date('2023-12-20','YYYY-MM-DD') AND TO_date('2023-12-23','YYYY-MM-DD');
